...
export function DashboardContent(): React.JSX.Element {
  ...
  const translateDashboard = useCallback(
    (key: string): string => tDashboard(key),
    [tDashboard],
  );
  const translateEntry = useCallback(
    (key: string): string => tEntry(key),
    [tEntry],
  );
  ...
  const resolveEntryMoods = useCallback(
    (entry: DiaryEntry): DashboardMood[] =>
      getEntryMoods(entry, resolveEmotionLabel),
    [resolveEmotionLabel],
  );
  ...
  const affirmationPool = useMemo(
    () => getAffirmationPool(translateDashboard),
    [translateDashboard],
  );
  ...
  const weekdayLabels = useMemo(
    () => getWeekdayLabels(translateDashboard),
    [translateDashboard],
  );
  ...
  const monthCells = useMemo<DashboardMonthCell[]>(() => {
    ...
    dashboardState.entriesByDate,
    dashboardState.monthGridDates,
  ...
  return (
    <section className="relative grid gap-ds-m pb-32">
      <DashboardHero
