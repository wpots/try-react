...
const firebaseConfig: FirebaseOptions = {
  apiKey:
    runtimeEnv.NEXT_PUBLIC_FIREBASE_API_KEY ?? runtimeEnv.VITE_FIREBASE_API_KEY,
  authDomain:
    runtimeEnv.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ??
    runtimeEnv.VITE_FIREBASE_AUTH_DOMAIN,
  projectId:
    runtimeEnv.NEXT_PUBLIC_FIREBASE_PROJECT_ID ??
    runtimeEnv.VITE_FIREBASE_PROJECT_ID,
  storageBucket:
    runtimeEnv.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET ??
    runtimeEnv.VITE_FIREBASE_STORAGE_BUCKET,
  messagingSenderId:
    runtimeEnv.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID ??
...
const isStorybookRuntime =
  runtimeEnv.STORYBOOK === true ||
  runtimeEnv.STORYBOOK === "true" ||
  runtimeEnv.STORYBOOK === "1";
...
if (missingVars.length > 0 && (isStorybookRuntime || isDevWithoutConfig)) {
  console.warn(
    `Firebase config missing: ${missingVars.join(", ")}. ` +
      "Using placeholder config (auth will not work).",
  );
}
